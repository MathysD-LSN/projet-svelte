...
export const load = async ({ params, fetch }) => {
  ...
  try {
    const responseMovie = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?language=en-US`, {
      headers: {
    ...
    const responseAlternativeMovies = await fetch(` https://api.themoviedb.org/3/movie/${movieId}/alternative_titles`, {
    ...
    const responseCredits = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?language=en-US`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
